...
export default function Chatbot() {
  ...
  if (!hasData) {
    return (
      <div className="relative w-full h-full flex items-center justify-center pl-25">
        ...
        <img src={nyangBot} alt="냥비서" className="absolute bottom-0 right-0 w-90 h-90" />
      </div>
    );
  }
  ...
  return (
    <div className="w-full grid grid-cols-2 gap-12">
      ...
      <div className="flex flex-col items-center">
        ...
        <ul className="space-y-4 w-[70%] pl-6">
          {sortedTodos.length > 0 ? (
            sortedTodos.map((item) => (
              <li key={item.id} className="flex flex-col gap-1">
                ...
                {item.description && (
                  <span className="text-sm text-gray-500 ml-8">{item.description}</span>
                )}
              </li>
            ))
          ) : (
